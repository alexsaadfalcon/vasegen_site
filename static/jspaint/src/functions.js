...
function update_title(){
	...
	if (is_pride_month) {
		$("link[rel~='icon']").attr("href", "/static/jspaint/images/icons/gay-es-paint-16x16-light-outline.png");
	}
}
...
function show_about_paint(){
	...
	if (is_pride_month) {
		$("#paint-32x32").attr("src", "/static/jspaint/images/icons/gay-es-paint-32x32-light-outline.png");
	}
...
function image_stretch_and_skew(){
	...
	const $RowInput = ($table, img_src, label_text, default_value, label_unit, min, max) => {
		...
		const $img = $(E("img")).attr({
			src: `/static/jspaint/images/transforms/${img_src}.png`,
			width: 32,
			height: 32,
		}).css({
			marginRight: "20px"
